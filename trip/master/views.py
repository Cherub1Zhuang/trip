...
def get_data_count(start=None,end=None):
    ...
    if end:
        query = query & Q(updated_at__lte=end)
...
def get_latest_order_stats(days=7):
    ...
    :return:
    """
    data_arr,amount_arr,count_arr=[],[],[]
    now_time=now()
    for x in range(days, 0, -1):
        # 日期
        calc_time=now_time+timedelta(hours=-(x-1) * 24)
        data_arr.append('{}号'.format(calc_time.day))
        ...
        # 订单金额
        amount_arr.append(result['amount'] or 0)
        # 订单数量
        count_arr.append(result['count'] or 0)
