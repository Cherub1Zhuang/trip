...
class RegisterForm(forms.Form):
    """用户注册"""
    username=forms.CharField(label='手机号',max_length=16,required=True,error_messages={
        'required':'请输入手机号码'
    })
    ...
    def do_register(self,request):
        ...
        try:
            ...
            user.last_login=now()
            user.save()
            user.add_login_record(username=user.username,ip=request.META.get('REMOTE_ADDR',''),source=source,version=version)
            return user, profile
